appli_Table<-function(data){
  # extract the needed information
  table = step$table
  dist_left = table[table[,1] == "dist_left",2]
 ...
 dist_left = dist_left +shift}
  ...
  if(dev_dir == 1){dist_bottom = dist_bottom + shift}else{dist_left = dist_left +shift}
  ...
  gcode = c(start_gcode,unlist(lapply(seq(path),function(k){
    ...
    for(band in seq(nrow(deriv_table))){
      gcode = c(gcode,
                paste0(if(dev_dir == 0){"G1 Y"}else{"G1 X"},dist_left+(band-1)*gap,"; go in band position"))
...
plot_step=function() {
    ...
    for(band in seq(nrow(deriv_table))){
      if(deriv_table$Use[band]){
        if(dev_dir == 0){
          symbols(x=dist_bottom+deriv_length/2,y=dist_left+(band-1)*gap,rectangles = rbind(c(deriv_length,if(nozzle == 0){3}else{0.25})),add = T,inches = F,bg = 1)
        }else if(dev_dir == 1){
          symbols(y=dist_bottom+deriv_length/2,x=dist_left+(band-1)*gap,rectangles = rbind(c(0.25,deriv_length)),add = T,inches = F,bg = 1)
        }
      }
    }
